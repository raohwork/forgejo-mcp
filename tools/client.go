...
// sendJSONRequestAllowEmpty behaves like sendSimpleRequest but tolerates an
// empty response body (HTTP 204), leaving respObj untouched in that case. This
// is needed for endpoints such as workflow dispatch, which may answer with
// either a JSON run-info object or no content at all.
func (c *Client) sendJSONRequestAllowEmpty(method, endpoint string, paramObj, respObj any) error {
	// Build complete URL
	u, err := url.Parse(c.base + endpoint)
	if err != nil {
		return fmt.Errorf("invalid URL: %w", err)
	...
	// A 204 (or otherwise empty body) is a valid success response here.
